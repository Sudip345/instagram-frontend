class ProfilePage {
    ...
    async updateProfileDisplay() {
        ...
        console.log('updateProfileDisplay called. profileUser.isLocked:', this.profileUser.isLocked);
    ...
    async loadCurrentUser() {
        try {
            ...
            console.log('Raw user data from loadCurrentUser:', user);
            this.loggedInUser = user;
            this.loggedInUser.isLocked = !!user.locked; 
            this.currentUser = user.username;
            console.log('loadCurrentUser - Current user:', this.currentUser, 'isLocked from backend:', user.isLocked, 'processed isLocked:', this.loggedInUser.isLocked);
            ...
            if (this.loggedInUser.profileAvatarID) {
                try {
                    ...
                    console.warn('Failed to load logged-in user avatar:', error);
                }
            }
    ...
    async loadOwnProfile() {
        try {
            ...
            const user = await response.json();
            console.log('Raw user data from loadOwnProfile:', user);
            ...
            console.log('loadOwnProfile - Profile loaded:', user.username, 'isLocked from backend:', user.isLocked, 'processed isLocked:', this.profileUser.isLocked);
        ...
        try {
            ...
            if (!response.ok) {
                ...
                if (user) {
                    ...
                    this.profileUser.bio = this.parseBioSafely(user.bio); // Safely parse bio
                    console.log(`Profile loaded with status ${response.status}:`, user);
                } else {
                    throw new Error(`HTTP error! status: ${response.status}`);
                }
            } else {
                ...
                this.profileUser.bio = this.parseBioSafely(user.bio); // Safely parse bio
                console.log('User profile loaded (200 OK):', user, 'isLocked:', this.profileUser.isLocked);
            }
    ...
    async checkAndUpdateFollowRequestButton(buttonElement) {
        try {
            const isFollowing = await this.checkIfUserFollowing(this.profileUser.username);
            const isRequestSent = await this.checkIfFollowRequestSent(this.profileUser.username);
            console.log('checkAndUpdateFollowRequestButton - isFollowing:', isFollowing, 'isRequestSent:', isRequestSent);
    ...
    setupEventListeners() {
        ...
        if (followActionBtn) {
            followActionBtn.addEventListener('click', async () => {
                if (this.profileUser.isLocked) {
                    console.log("profileUser.isLocked is true, toggling follow request");
                    await this.toggleFollowRequest();
                } else {
                    console.log("profileUser.isLocked is false, toggling follow");
    ...
    async toggleProfileLockStatus(lock) {
        console.log('toggleProfileLockStatus called with lock:', lock);
        console.log('profileUser.isLocked BEFORE API call:', this.profileUser.isLocked);
        ...
        try {
            ...
            console.log(`toggleProfileLockStatus - API call to ${endpoint} responded with status: ${response.status} ${response.statusText}, ok: ${response.ok}`);
            ...
            console.log('toggleProfileLockStatus - profileUser.isLocked AFTER local update:', this.profileUser.isLocked);
            ...
            console.error(`Error ${actionText.toLowerCase()} profile:`, error);
    ...
    async updateMoreOptionsModalDisplay() {
        console.log('updateMoreOptionsModalDisplay called. profileUser.isLocked:', this.profileUser.isLocked);
        const toggleProfilePrivacyOption = document.getElementById('toggleProfilePrivacy');
    ...
    async loadPosts() {
        ...
        try {
            ...
            if (response.status === 423) {
                ...
                console.log('Profile is locked. Not loading posts.');
            ...
            if (userData.list && Array.isArray(userData.list) && userData.list.length > 0) {
                console.log('Found post IDs in user data:', userData.list);
    ...
    async openPostViewer(clickedPost) {
        console.log('Opening post viewer for:', clickedPost);
        try {
            ...
            console.error('Error in openPostViewer:', error);
            this.showMessage('Failed to open post: ' + error.message);
        }
    }

    openProfilePictureViewer() {
        // Find if the user whose profile is being viewed has stories
        ...
        if (hasStories) {
            // If stories exist, for any user, open the story modal.
    ...
    async saveProfile() {
        ...
        try {
            ...
            console.log('Bio update response status:', bioResponse.status);
            if (!bioResponse.ok) {
            ...
            console.log('Links update response status:', linksResponse.status);
            if (!linksResponse.ok) {
    ...
    async toggleFollowRequest() {
        ...
        try {
            ...
            console.error(`Error toggling follow request status:`, error);
            this.showError(errorMessage + ' Please try again.');
        }
    }
    ...
    async renderFollowRequest(username, container) {
        try {
            ...
            console.error('Error rendering follow request:', error);
        }
    }
    ...
    setupPostModalEventListeners(modal, postId, post) {
        ...
        console.log(likeBtn, commentBtn, saveBtn, followBtn, closeBtn, commentInput, commentPostBtn);
  ...
  checkIfUserLiked(item) {
    ...
    async createCommentTree(comment, depth = 0, postId) {
        ...
        // Format time
        let timeAgo = this.getTimeAgo(new Date(comment.time)|| new Date());
        ...
        try {
            ...
            if (parentCommentDiv) {
                ...
                if (repliesContainer && (this.expandedReplies.has(parentCommentId) || !viewRepliesBtn)) {
                    ...
                    if (replyElement) {
                        repliesContainer.prepend(replyElement);
                        // Ensure the reply input is hidden after successful submission
    ...
    async fetchAllStories() {
        try {
            ...
            // The backend already sorts, so no need for client-side sorting.
            // Just convert the Map values to an array.
   ...
   async  updateViewStoryButton(story) {
    ...
    async postStoryWithCroppedFile(file) {
        try {
            ...
            console.error('Error during postStoryWithCroppedFile flow:', error);
        } finally {
