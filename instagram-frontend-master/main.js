class InstagramClone {
    ...
    constructor() {
        ...
        console.log('InstagramClone initializing...');
    ...
    async loadUserProfile() {
        ...
        try {
            ...
            console.log('User profileAvatarID:', this.loggedInUser.profileAvatarID);
            if (this.loggedInUser.profileAvatarID) {
                try {
                    console.log(`Fetching avatar from: ${this.baseUrl}/files/get-files/file-id=${this.loggedInUser.profileAvatarID}`);
                    ...
                    if (avatarResponse.ok) {
                        ...
                        console.log('Avatar loaded successfully:', this.loggedInUserAvatar);

                        // Update profile display after avatar loads
                        this.updateProfileDisplay();
                    } else {
                        console.error('Failed to fetch avatar, status:', avatarResponse.status);
                    }
                } catch (e) {
                    console.warn('Failed to load user avatar:', e);
                }
            } else {
                console.log('User has no profileAvatarID, using default avatar');
        ...
        try {
            ...
            if (!response.ok) {
                if (response.status === 404) {
                    console.log(`No common followers found for ${username}.`);
                    return [];
                }
                throw new Error(`HTTP error! status: ${response.status}`);
            }
            const commonFollowers = await response.json();
            console.log(commonFollowers, "common followe");
            return commonFollowers; // Assuming this returns an array of usernames
        } catch (error) {
            console.error(`Error fetching common followers for ${username}:`, error);
            return [];
        }
    }
    ...
    async loadFeed(reset = false) {
        if (this.isLoadingPosts || !this.hasMorePosts) {
            console.log('Already loading posts or no more posts to load.');
            return;
        }
        ...
        console.log(`Loading feed page ${this.currentPage} with size ${this.pageSize}...`);
        try {
            ...
            if (response.status === 401 || response.status === 403) {
                console.warn('Unauthorized in loadFeed, redirecting to login...');
            ...
            const newPosts = await Promise.all(blogs.map(async blog => {
                if (Array.isArray(blog.fileID)) {
                    blog.imageUrls = await Promise.all(blog.fileID.map(async fileObj => {
                        ...
                        if (!fileRes.ok) {
                            console.error(`Failed to fetch file for id: ${id}`);
                            return null;
                        }
            ...
            if (this.posts.length === 0) {
                ...
                console.log('No posts found after load, displaying message.');
    ...
    updateProfileDisplay() {
        console.log('Updating profile display with:', this.userProfile);
        try {
    ...
    async renderFeed(postsToRender = []) { // Renamed newPosts to postsToRender for clarity
        console.log('Rendering feed with posts:', postsToRender);
        ...
        for (const post of postsToRender) {
            ...
            console.log('Appended post element to feed container:', post.stringID);
        }
    }
    ...
    async createPostElement(post) {
        ...
        console.log('Post', post.stringID, 'saved status:', isSaved);
        ...
        postElement.innerHTML = `
            <div class="post-header">
                ...
                <span class="post-time-header">${this.getTimeAgo(new Date(post.localDateTime))}</span>
        ...
        console.log('Rendered save button for post', post.stringID, 'with saved state:', isSaved);
        ...
        likeBtn.addEventListener('click', async () => {
            ...
            try {
                let response;
                if (newLikedState) {
                    response = await fetch(`${this.baseUrl}/user/add-likes/${post.stringID}`, { method: 'POST', credentials: 'include' });
                } else {
        ...
        saveBtn.addEventListener('click', async (e) => {
            e.preventDefault();
            e.stopPropagation();
            console.log('Save button clicked for post:', post.stringID);
    ...
    async addReply(postId, commentId, text) {
        try {
            console.log('addReply called with:', { postId, commentId, text });
            ...
            img.onload = () => {
                ...
                canvas.toBlob((blob) => {
                    if (blob) {
                        ...
                        console.log(`Optimized ${file.name}: ${this.formatFileSize(file.size)} → ${this.formatFileSize(optimizedFile.size)}`);
    ...
    renderCommentsPreview(commentsList) {
        ...
        return validComments.slice(0, 2).map(comment => {
            const timeAgo = comment.time ? this.getTimeAgo(new Date(comment.time)) : '';
            return `
            <div class="comment-preview">
                <div class="comment-preview-line1">
                    <span class="comment-username">${this.currentUser}</span>
    ...
    setupModal() {
        ...
        if (!modal || !openBtn || !imageInput || !selectedImagesDiv || !captionInput || !shareBtn) {
            console.warn('Modal elements not found:', {
                ...
                selectedImagesDiv: !!selectedImagesDiv,
    ...
    setupCommentsModal() {
        console.log('Setting up comments modal...');
        ...
        if (!modal || !commentInput || !submitComment || !commentsContainer) {
            console.warn('Comments modal elements not found', { modal, commentInput, submitComment, commentsContainer });
            return;
        }
        ...
        submitComment.onclick = async (e) => {
            ...
            e.stopPropagation();
            const text = commentInput.value.trim();
            console.log('Post button clicked, text:', text, 'currentPostId:', this.currentPostId);
            if (text && this.currentPostId) {
                commentInput.disabled = true;
                submitComment.disabled = true;
                submitComment.textContent = 'Posting...';
    ...
    setupLikesModal() {
        console.log('Setting up likes modal...');
        const modal = document.getElementById('likes-modal');

        if (!modal) {
            console.warn('Likes modal not found');
            return;
        }
        ...
        modal.onclick = (e) => {
            if (e.target === modal) {
                ...
                modal.classList.remove('active');
                document.body.style.overflow = '';
                console.log('Likes modal closed by backdrop click');
            }
        };
        ...
        document.addEventListener('keydown', (e) => {
            if (e.key === 'Escape' && (modal.style.display === 'block' || modal.classList.contains('active'))) {
                ...
                modal.classList.remove('active');
                document.body.style.overflow = '';
                console.log('Likes modal closed by Escape key');
            }
        });
    }
    ...
    async showLikesModal(postId) {
        ...
        const modal = document.getElementById('likes-modal');
        if (!modal) {
            console.error('Likes modal not found');
            return;
        }
    ...
    async renderLikesModal(postId) {
        console.log('Rendering likes for post:', postId);
        ...
        try {
            ...
            console.log('Users who liked:', usersMap);
        ...
        try {
            ...
            if (profileAvatarID) {
                try {
                    ...
                    console.warn('Failed to load avatar for user:', username, e);
                }
            }
            ...
            console.error('Error rendering like user:', error);
        }
    }
    ...
    async showPostModal(post) {
        try {
            console.log('Showing post modal for:', post);
            ...
            if (!modal) {
                console.error('Post modal not found in DOM');
                return;
            }
            ...
            if (!followBtn) {
                ...
                if (!followBtn) {
                    ...
                    if (headerDiv) {
                        ...
                        console.log('Created missing follow button');
                    }
                }
            }
            ...
            if (!followBtn) console.warn('Follow button not found in modal');
            if (!likeBtn) console.warn('Like button not found in modal');
            if (!saveBtn) console.warn('Save button not found in modal');
            ...
            if (modalCaptionTime) modalCaptionTime.textContent = this.getTimeAgo(new Date(post.localDateTime));
            if (modalLikes) modalLikes.textContent = `${likesList.length} likes`;
            if (modalTime) modalTime.textContent = this.getTimeAgo(new Date(post.localDateTime));
            ...
            if (followBtn) {
                ...
                console.warn('Follow button not found in modal');
            }
            ...
            if (commentsSection) {
                ...
                if (commentList) {
                    ...
                    console.warn('Comment list not found in post modal');
                }
            ...
            this.handleCommentLikeClickInPostModal = async (e) => {
                ...
                if (likeBtn) {
                    ...
                    try {
                        ...
                        console.error('Error toggling comment like in post modal:', error);
                        // Revert UI on error
                        likeBtn.setAttribute('data-liked', String(isCurrentlyLiked));
                        if (svg) {
                            svg.setAttribute('fill', isCurrentlyLiked ? '#ff3040' : 'none');
    ...
    async refreshPostModalComments(postId) {
        ...
        if (!commentList) {
            console.error('Post modal comment list not found for refresh.');
            return;
        }
        ...
        try {
            ...
            console.error('Error refreshing post modal comments:', error);
    ...
    async toggleFeedFollow(author, followBtn) {
        ...
        try {
            ...
            console.error('Error in toggleFeedFollow:', error);
    ...
    async togglePostModalFollow(username, followBtn) {
        ...
        try {
            ...
            console.error('Error in togglePostModalFollow:', error);
            // Revert UI on error by re-checking the state
                ...
                if (comment && comment.comment && comment.author) {
                    ...
                    commentElement.innerHTML = `
                        <div class="comment-main-row">
                            ...
                            <span class="comment-time">${comment.time ? this.getTimeAgo(new Date(comment.time)) : ''}</span>
                        </div>
    ...
    async toggleSavePost(postId, saveBtn) {
        try {
            const isSaved = saveBtn.getAttribute('data-saved') === 'true';
            console.log('Toggle save - current state:', isSaved, 'for post:', postId);
    ...
    async createCommentTree(comment, depth = 0, postId) {
        ...
        // Format time
        let timeAgo = '';
        timeAgo = this.getTimeAgo(new Date(comment.time));
...
`;
            ...
            viewRepliesBtn.onclick = async (e) => {
                ...
                if (!this.expandedReplies.has(comment.stringID)) {
                    ...
                    try {
                        ...
                        console.error('Failed to fetch replies for comment', comment.stringID, err);
                    }
    ...
    setupEditProfileModal() {
        console.log('Setting up edit profile modal...');
        ...
        if (!modal || !saveBtn || !profileForm) {
            console.warn('Edit profile modal elements not found');
            return;
        }
    ...
    setupPostOptionsModal() {
        console.log('Setting up post options modal...');
        ...
        if (!modal || !closeBtn || !deleteOption) {
            console.warn('Post options modal elements not found');
            return;
        }
    ...
    setupReportOptionsModal() {
        console.log('Setting up report options modal...');
        ...
        if (!modal || !closeBtn || !optionsContainer) {
            console.warn('Report options modal elements not found');
            return;
        }
    ...
    async openReportOptionsModal(author, postId, event) {
        if (!event) {
            console.warn('openReportOptionsModal called without an event, blocking to prevent opening on page load.');
            return;
        }
        console.log('Opening report options modal for:', {
            author,
            postId
        });
        const modal = document.getElementById('report-options-modal');
        ...
        if (!modal || !optionsContainer) {
            console.error('Report options modal or container not found!');
            return;
        }

        optionsContainer.innerHTML = ''; // Clear previous options

        // Option 1: Report this post
    ...
    setupReportDescriptionModal() {
        console.log('Setting up report description modal...');
        ...
        if (!modal || !closeBtn || !submitReportBtn || !reportDescriptionInput || !reportOptionsContainer || !otherDescriptionContainer) {
            console.warn('Report description modal elements not found');
            return;
        }
    ...
    openReportDescriptionModal(targetId, type) {
        console.log('Opening report description modal for:', { targetId, type });
        const modal = document.getElementById('report-description-modal');
        ...
        if (!modal || !reportOptionsContainer || !reportDescriptionInput || !otherDescriptionContainer) {
            console.error('Report description modal elements not found!');
            return;
        }
    ...
    async getCurrentUserLikeId(blogId) {
        ...
        if (!res.ok) {
            console.log(`getCurrentUserLikeId: Response not OK (${res.status}) for blogId: ${blogId}`);
            return null;
        }
        ...
        if (!text) {
            ...
            console.log('getCurrentUserLikeId received an empty response, meaning user has not liked this post.');
            return null;
        }

        try {
            const data = JSON.parse(text);
            console.log('getCurrentUserLikeId response:', data);
            ...
            console.warn('getCurrentUserLikeId: Found data object but no valid ID field.', data);
            return null;
        } catch (e) {
    ...
    async togglePostModalLike(postId, likeBtnElement, modal) {
        ...
        try {
            ...
            console.error('Error toggling post modal like:', error);
            // Revert UI on error
    ...
    async checkCorsConfig() {
        try {
            ...
            console.error('CORS check failed due to network error or misconfiguration:', error);
            return false;
        }
    }
    ...
    setupFollowRequestsModal() {
        ...
        if (!modal) {
            console.warn('Follow requests modal not found.');
            return;
        }
    ...
    async loadFollowRequests() {
        ...
        if (!requestsContainer) {
            console.error('Follow requests list container not found.');
            return;
        }
        ...
        try {
            ...
            if (response.status === 401 || response.status === 403) {
                console.warn('Unauthorized in loadFollowRequests, redirecting to login...');
                window.location.href = 'login.html';
            ...
            console.log('Received follow requests:', receivedRequests);
    ...
    async renderFollowRequest(username, container) {
        try {
            ...
            console.error('Error rendering follow request for', username, ':', error);
        }
    }
    ...
    async updateFollowRequestsDot() {
        ...
        if (!followRequestsBtn || !icon) {
            console.warn('Follow requests main button or icon not found.');
            return;
        }
        ...
        try {
            ...
            console.error('Error updating follow request icon:', error);
    ...
    async postStoryWithCroppedFile(file) {
        try {
            ...
            console.log('File ID response:', fileIdResponse);
            ...
            console.log('Post story response:', postStoryResponse);
            const storyResponseData = await postStoryResponse.json();
            console.log('Post story response data:', storyResponseData);
    ...
    async fetchAllStories() {
        try {
            ...
            console.log('All stories after merge:', flattenedStories);
            ...
            sortedStories.sort((a, b) => {
                ...
                const aLatestTime = Math.max(...a.stories.map(s => new Date(s.localDateTime).getTime()));
                const bLatestTime = Math.max(...b.stories.map(s => new Date(s.localDateTime).getTime()));
    ...
    async openStoryViewersModal(story) {
        ...
        try {
            if (story && story.viewedBy) {
                console.log('Raw story.viewedBy:', story.viewedBy); // Diagnostic log
                const uniqueViewers = [...new Set(story.viewedBy)]; // Ensure unique viewers
                console.log('Unique viewers after Set conversion:', uniqueViewers); // Diagnostic log
                ...
                console.log('Rendering unique viewers:', uniqueViewers); // Added diagnostic log

                for (const username of uniqueViewers) {
                    console.log(`Fetching avatar for viewer: ${username}`);
                    const avatarUrl = await this.getUserAvatar(username);
                ...
                console.log('Number of viewer items in DOM after rendering:', viewersList.children.length); // Added diagnostic log
            } else {
    ...
    async renderStoryModalContent() {
        ...
        if (activeStoryGroup && activeStory) {
            ...
            if (storyUsernameSpan) {
                const timeAgo = this.getTimeAgo(new Date(activeStory.createdAt));
    ...
    setupStoryModalEventListeners() {
        ...
        if (!storyModal) {
            console.warn('Story modal not found, skipping event listeners setup.');
            return;
        }
    ...
    async setupPeopleSuggestions() {
        ...
        if (!modal || !list) {
            console.warn('People suggestions modal elements not found');
            return;
        }
        ...
        try {
            ...
            console.error('Error setting up people suggestions:', error);
        }
    }
...
document.addEventListener('DOMContentLoaded', () => {
    console.log('DOM loaded, creating instance...');
